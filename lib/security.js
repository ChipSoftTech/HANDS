			...
			if (parts.length == 2) {
				...
				if (/^Bearer$/i.test(scheme)) {
					...
					try {
						...
						case "GET": //Read
							if (decoded.permissions[channel]
								 && (decoded.permissions[channel].admin || decoded.permissions[channel].read)) {
								authorized == value;

								if (value == 1 && !decoded[channel].admin) {
									req.query.createdby == email;
								}
							}
							break;
						case "PUT": //Update
							if (decoded.permissions[channel]
								 && (decoded.permissions[channel].admin || decoded.permissions[channel].update)) {
								authorized = value;
						...
						case "DELETE": //Delete
							if (decoded.permissions[channel]
								 && (decoded.permissions[channel].admin || decoded.permissions[channel].delete )) {
								authorized = value;

...
server.post('/api/v1/:_channel/hands/register', function (req, res, next) {
	...
	database.create(req.params._channel, "hands", "tokens", req.body, req.params._createdby, function (err, data) {
		if (err) {
 ...
 * Add Permissions
 ****************************************/
server.post('/api/v1/:_channel/hands/tokens/:_id?', function (req, res, next) {
 ...
 * Read
 ****************************************/
server.get('/api/v1/:_channel/hands/tokens/:_id?', function (req, res, next) {
	debug("GET-by id-request received");

	database.read(req.params._channel, "hands", "tokens", req.params._id, null, null, null, function (err, data) {
		if (err) {

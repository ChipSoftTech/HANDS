...
server.post('/api/v1/:_channel/:_db/:_collection', function (req, res, next) {
	...
	database.create(req.params._channel, req.params._db, req.params._collection, req.body, req._currentToken.email, function (err, data) {
		if (err) {
...
server.get('/api/v1/:_channel/:_db/:_collection', function (req, res, next) {
	...
	database.read(req.params._channel, req.params._db, req.params._collection, null , req._onlyCreatedby, req.query.query, req.query.options, function (err, data) {
		if (err) {
			return next(new restify.BadRequestError(err.toString()));
		}
...
server.get('/api/v1/:_channel/:_db/:_collection/:_id?', function (req, res, next) {
	...
	database.read(req.params._channel, req.params._db, req.params._collection, req.params._id, req._onlyCreatedby, req.query.query, req.query.options, function (err, data) {
		if (err) {
...
server.put('/api/v1/:_channel/:_db/:_collection/:_id', function (req, res, next) {
	...
	database.update(req.params._channel, req.params._db, req.params._collection, req.params._id, req._onlyCreatedby, req._currentToken.email, req.body, function (err, data) {
		if (err) {
			if(err == "Error: 404 Not Found") {
				return next(new restify.NotFoundError(err.toString()));
			} else {
